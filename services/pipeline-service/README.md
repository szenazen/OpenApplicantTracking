...
- **Internal slice API (JWT + `x-account-id` matching path):** `/api/slice/pipeline/accounts/:accountId/pipelines` and related CRUD — same JSON shapes as the monolith’s `/api/pipelines` routes. **`GET .../jobs`** — paginated job list (monolith-aligned); pilot combo with pipelines in **one** service/DB vs separate **Job Service** in [`design/ATS-design.drawio.xml`](../../design/ATS-design.drawio.xml) ([`design/strangler-vs-ats-diagram.md`](../../design/strangler-vs-ats-diagram.md)).
