...
| **Regional ATS** data: **jobs**, **pipelines**, **applications** cylinders | Slice DB holds **minimal** jobs + pipelines + statuses + application stubs (for stage-delete rules and list counts); full job/Kanban payloads still come from **regional DB via `apps/api`** for unmigrated routes. |
