...
The target architecture in `design/ATS-design.drawio.xml` places a **Web BFF** (and separate deployable services) in front of regional and global backends, with one browser/API entry and **service ownership** of data. The OpenApplicantTracking repo also keeps [`apps/api`](../../apps/api) as a **modular monolith** for speed of development and as a **parallel reference** when extracting slices.
...
3. Retain **`apps/api` unchanged in role** — it remains the modular monolith and primary implementation until more domains are split out; extracted services (e.g. `account-service`) are called through the BFF.
...
- **Positive:** `apps/*` continue to be the monolith “backup” and dev default without being replaced by a second full stack.
